      ...
      <table class="table table-bordered table-hover table-striped">
        ...
        <tbody>
          ...
          <tr>
            ...
            <td class="text-center">
              <img src="{{ asset("images/coupons/".$row->image) }}" class="img-fluid" alt="{{ $row->image }}">
            </td>
            <td>{{ $row->name }}</td>
            <td>{{ $row->code }}</td>
            <td>{{ $row->qty }}</td>
            ...
            <td class="text-center">
              <div class="d-inline-flex">
                <a href="{{ route('admin.coupon.show', $args) }}" class="btn btn-primary btn-xs mr-1">
                  <i class="fas fa-eye"></i>
                </a>
                <a href="{{ route('admin.coupon.restore', $args) }}" class="btn btn-success btn-xs mr-1">
                  <i class="fas fa-undo-alt"></i>
                </a>
                <form action="{{ route('admin.coupon.destroy', $args) }}" method="POST" class="d-inline">
                  @csrf
                  @method('delete')
